...
- 2026-06-10 Polvert patent certificate asset direction: create a public-safe patent certificate image under `prototype/assets/polvert/patent-certificate/` using the existing mosaic-safe certificate as the privacy baseline, highlighting only the invention title `오른손 골퍼용 오른손 장갑`, and compositing repeated diagonal Polvert logo watermarks from the official transparent logo. The design is documented in `docs/superpowers/specs/2026-06-10-polvert-patent-certificate-edit-design.md`.
...
- 2026-05-28 Polvert PV01 reference memory: use `docs/agent-studio/polvert-pv01-rh-microfiber-feature-analysis.md` as the structure guide for `PV01_MICROFIBER / RH_MEN` generation. The right-hand microfiber glove is an open-palm, half-finger glove with exposed fingertips, narrow black finger bands, vertical finger perforations, a curved black hand-back line, a black rounded hand-back patch, and a palm-side wrist label. Front/palm wearing shots and back/patch shots must be generated from different structural rules; do not generate PV01 as a full-finger, closed-palm, or mixed-front/back glove. Analyze the glove shape and pass the checklist first, generate blank-patch structure tests second, and composite the real Polvert logo only after the glove form is accepted.
...
- 2026-06-05 responsive hero signal rule: show the hero distance feedback as a projected laser-style `퍼팅 거리 표시 / 2.3M` readout near the ball's address point across all browser widths. Do not restore the full `신호/루틴/라인업` dashboard or a button-like capsule in the hero; the visual should make it obvious that the distance remains where the golfer can keep their head down.
